...
namespace CrystalByRiya.Pages
{
    public class checkoutModel : PageModel
    {
        ...
        private void EnsureCheckoutDefaults()
        {
            ...
            // Always default checkout information to India.
            Detail.Country = "India";

            var indiaStates = GetStatesForCountry("India");
            if (indiaStates.Count == 0)
            {
                return;
            }

            if (string.IsNullOrWhiteSpace(Detail.State) || !indiaStates.Contains(Detail.State, StringComparer.OrdinalIgnoreCase))
            {
                Detail.State = indiaStates[0];
            }
        }
